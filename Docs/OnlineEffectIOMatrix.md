...
| PublicSelection | 4 |
| MultiStepRestZoneSelection | 2 |
| OpponentChoiceSelection | 1 |
| DeckOrderAuthoritySelection | 1 |
| PrivateSelection | 8 |
| PersistentStatus | 11 |
...
| PublicSelectionAutoSingle | Candidate count 1 resolves automatically; candidate count 2+ opens selection UI. | Host auto-resolves one candidate; otherwise creates `SelectionRequestDelta`. | `idol.active.fullHealOneControlled`, `character.active.forceBattleTargetAnywhere` |
...
| `character.active.forceBattleTargetAnywhere` | CharacterActive | `EffectManager.ResolveForceBattleTargetAnywhereEffect`; online `CreateForceBattleTargetAnywhereSelectionOrResultFromExternal` / `CreateForceBattleTargetAnywhereFinalResult` | source slot `characterOwner`; source character active | Public face-up opponent field character target; 1 candidate auto-resolves, 2+ opens selection | Forced incoming collab using selected target as attacker and source as defender; collab result may rest defeated characters and merge supported OnRest simple Deltas | SelectionRequest, StartCollabResult, Viewer if cost, ActionState, supported OnRest simple Delta, Log | OnSuccess / AllowCancel only when selection UI opens | PublicSelectionAutoSingle / B | Forced attacker is selected target `characterOwner`; defender/action source is active source `characterOwner`. Battle location is the source slot; do not replace card ownership with `slot.owner`. |
...
| `character.onAppear.callFromRestByTagToEmptyPlatforms` | OnAppear | `EffectManager.ResolveCallFromRestByTagToEmptyPlatformsEffect` | appeared character `characterOwner` | Owner rest-zone card choice, then empty owned platform target; repeat up to local `maxCount` | Rest character to field, possible OnAppear chain | SelectionRequest, CardZoneMove, supported OnAppear simple Delta, Log | No for trigger / AllowCancel finishes partial local sequence | MultiStepRestZoneSelection / F | Rest zone is public in current local UI, but this is not pure field-target PublicSelection: it chains card choice, destination choice, repeat/partial-finish, and OnAppear follow-up. Destination slot owner is platform, characterOwner remains effect owner. |
...
| `content.forceOpponentFlipOrSack` | Content | `EffectManager.ResolveForceOpponentFlipOrSackEffect` | hand content owner actor | Public opponent face-down field slot selection, then opponent pay-or-sack decision | Flip target or send/sack per local rule | SelectionRequest, OpponentDecisionAction, CardZoneMove, CardReveal, Viewer, ActionState, Log | OnSuccess / DuringSelection | OpponentChoiceSelection / F | Not pure PublicSelection: after actor selects a public slot, the target owner must choose whether to pay double appear cost and flip. Face-down card id may be hidden until flip; public target is slot id. |
...
| `content.returnUpToNFromRestToDeck` | Content | `EffectManager.ResolveReturnUpToNFromRestToDeckEffect` | hand content owner actor | Owner rest-zone card sequence up to N | Rest cards move to deck, maybe shuffle/top/bottom per params | SelectionRequest, CardZoneMove, DeckOrder, Viewer, ActionState, Log | OnSuccess / AutoSingle, otherwise DuringSelection | DeckOrderAuthoritySelection / C | Not pure PublicSelection: Host must serialize authoritative deck insertion/shuffle order and support sequential up-to-N selection. Rest zone card owner controls deck owner. |
| `content.silenceCharacterCollabThisTurn` | Content | `EffectManager.TryResolveEffect`; online `TryStartSilenceCharacterCollabThisTurnFromExternal` | hand content owner actor | Public face-up field character target | Silence collab effects this turn; source content to rest | SelectionRequest, Status, CardZoneMove, Viewer, ActionState, Log | OnSuccess / DisallowCancel | PersistentStatusSelection / D | Selection flow is online implemented, but the result is persistent turn-scoped status, not pure PublicSelection closure. Status target is selected character slot/card; characterOwner may differ from slot owner. |
| `idol.active.callFromRestByTagThenDonateViewers` | IdolActive | `EffectManager.ResolveCallFromRestByTagThenDonateViewersEffect` | `action.actor` idol | Owner rest-zone card choice, then empty owned platform target | Call tagged rest character to field, donate viewers to opponent, possible OnAppear chain | SelectionRequest, CardZoneMove, Viewer, ActionState, supported OnAppear simple Delta, Log | OnSuccess / AllowCancel during either selection step | MultiStepRestZoneSelection / F | Not pure field-target PublicSelection: requires rest-zone card choice, destination choice, required viewer total check, donate viewer transfer, and OnAppear follow-up. Source idol must be actor idol; summoned characterOwner is actor. |
...
- `character.active.forceBattleTargetAnywhere`: selected opponent target becomes forced attacker; active source stays defender/action source. Collab rest/move owner uses each slot `characterOwner`, while battle location uses the source slot.
...
## PublicSelection Remaining Audit

| effectRef | Current Type | Local Handler | Candidate Visibility | Result Shape | Can Close In PublicSelection? | Decision | Reason |
|---|---|---|---|---|---|---|---|
| `character.active.modifyTaggedOnBoard` | PublicSelectionRequired | `EffectManager.ResolveModifyCharacterStatsEffectWithResult` / `BuildModifyTaggedOnBoardRequest` | Public face-up field character | `FieldStatDelta`, `ViewerDelta`, `ActionStateDelta` | Yes | Implemented | Required selection even with one candidate; owner uses `characterOwner`. |
| `idol.active.fullHealOneControlled` | PublicSelectionAutoSingle | `EffectManager.ResolveIdolFullHealOneControlledEffect` | Public controlled face-up field character | `FieldStatDelta`, `ViewerDelta`, `ActionStateDelta` | Yes | Implemented | One candidate auto-resolves; 2+ opens selection; controlled means `characterOwner == actor`. |
| `character.active.forceBattleTargetAnywhere` | PublicSelectionAutoSingle | `EffectManager.ResolveForceBattleTargetAnywhereEffect` | Public face-up opponent field character | `StartCollabResult`, `ViewerDelta`, `ActionStateDelta`, supported OnRest deltas | Yes | Implemented | One candidate auto-resolves; 2+ opens selection; selected target is forced attacker and source is defender. |
| `character.active.adjacentHpDownAndTensionUpForTag` | PublicAutoResolveMultiTarget | `EffectManager.ResolveModifyCharacterStatsEffectWithResult` / `BuildAdjacentHpDownAndTensionUpRequest` | Public adjacent face-up field characters | `FieldStatDelta`, `ViewerDelta`, `ActionStateDelta`, `CardZoneMoveDelta`, supported OnRest deltas | Yes | Implemented | No selection UI; all valid local candidates are resolved by Host. |
| `content.moveOwnCharToEmptyOrBattleIfTagged` | PublicSelection multi-step | `EffectManager.ResolveMoveOwnCharToEmptyOrBattleIfTaggedEffect` | Public own tagged field character, public destination slot | `CardZoneMoveDelta` or `StartCollabResult`, `ViewerDelta`, `ActionStateDelta` | Yes | Implemented | Multi-step public field selection is already online implemented; owner/remap risk remains around destination `slot.owner` vs moving `characterOwner`. |
| `content.silenceCharacterCollabThisTurn` | PersistentStatusSelection | `BattleManager.TryStartSilenceCharacterCollabThisTurnFromExternal` | Public face-up field character | `StatusDelta`, `CardZoneMoveDelta`, `ViewerDelta`, `ActionStateDelta` | No, status-backed | Reclassified out | Selection path is implemented, but the result is a turn-scoped persistent status and belongs to PersistentStatus follow-up, not pure PublicSelection closure. |
| `content.forceOpponentFlipOrSack` | OpponentChoiceSelection | `EffectManager.ResolveForceOpponentFlipOrSackEffect` | Public opponent face-down field slot, then target-owner decision | `OpponentDecisionAction`, `CardRevealDelta`, `CardZoneMoveDelta`, `ViewerDelta`, `ActionStateDelta` | No | Reclassified out | Actor slot selection is public, but local result requires the opponent to choose pay-and-flip or sack; this needs a dedicated opponent-decision protocol, not a new Delta type. |
| `character.onAppear.callFromRestByTagToEmptyPlatforms` | MultiStepRestZoneSelection | `EffectManager.ResolveCallFromRestByTagToEmptyPlatformsEffect` | Owner rest-zone card and empty owned platform slot | `CardZoneMoveDelta`, supported OnAppear deltas | No | Reclassified out | Trigger can repeat up to `maxCount`, allow partial finish, and chain OnAppear; not covered by stable field-target PublicSelection. |
| `idol.active.callFromRestByTagThenDonateViewers` | MultiStepRestZoneSelection | `EffectManager.ResolveCallFromRestByTagThenDonateViewersEffect` | Owner rest-zone card and empty owned platform slot | `CardZoneMoveDelta`, `ViewerDelta`, `ActionStateDelta`, supported OnAppear deltas | No | Reclassified out | Requires rest-zone card choice, destination choice, combined cost/donation check, donation transfer, and OnAppear chain. |
| `content.returnUpToNFromRestToDeck` | DeckOrderAuthoritySelection | `EffectManager.ResolveReturnUpToNFromRestToDeckEffect` | Owner rest-zone cards | `CardZoneMoveDelta`, `DeckOrderDelta`, `ViewerDelta`, `ActionStateDelta` | No | Reclassified out | Rest-zone selection is public locally, but Host must serialize authoritative deck insertion/shuffle order and sequential up-to-N selection. |

## PublicSelection Closure Summary
...
Reclassified out of pure PublicSelection:
- `content.silenceCharacterCollabThisTurn` -> `PersistentStatusSelection`
- `content.forceOpponentFlipOrSack` -> `OpponentChoiceSelection`
- `character.onAppear.callFromRestByTagToEmptyPlatforms` -> `MultiStepRestZoneSelection`
- `idol.active.callFromRestByTagThenDonateViewers` -> `MultiStepRestZoneSelection`
- `content.returnUpToNFromRestToDeck` -> `DeckOrderAuthoritySelection`

Remaining PublicSelection:
- None for the stable pure public field-target common flow.

Closure decision:
PublicSelection common flow now covers Required, AutoSingle, AutoResolveMultiTarget, public field multi-step movement, and StartCollabResult-backed selection. Remaining non-implemented effects are not pure PublicSelection and should be handled in later PersistentStatus, OpponentDecision, MultiStepRestZoneSelection, DeckOrderAuthority, PrivateSelection, HostRng, or PostCollab phases.

## Recommended Implementation Order

1. Implement `PersistentStatusSelection` and StatusDelta expiry/registry validation for status-backed public target effects.
2. Implement `OpponentChoiceSelection` for effects where a public target is chosen by one player but the target owner makes the next local decision.
3. Implement `MultiStepRestZoneSelection` with rest-zone card descriptors, destination slot selection, partial-finish policy, and OnAppear merge.
4. Implement `DeckOrderAuthoritySelection` after deck insertion/shuffle order serialization is fixed.
5. Implement remaining PrivateSelection after public request/response, status, rest-zone, and deck-order policies are stable.
