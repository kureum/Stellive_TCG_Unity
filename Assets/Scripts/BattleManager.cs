...
public class BattleManager : MonoBehaviour
{
    ...
    private bool onlineUseCharacterActiveRequestPending;
    private bool onlineUseIdolActiveRequestPending;
    ...
    private class PendingOnlineSelection
    {
        public string requestId = "";
        public string effectRef = "";
        public string sourceCardInstanceId = "";
        public string selectedSourceCharacterInstanceId = "";
        public string selectedSourceSlotId = "";
        ...
        public int originalViewerCost;
        public bool viewerCostPaid;
        public bool actionUseCommitted;
        public string currentResolveStep = "";
        public bool basicFallbackAvailable;
        public bool cancelAllowed;
        public readonly List<PendingOnlineTrigger> deferredTriggers = new List<PendingOnlineTrigger>();
        public readonly HashSet<string> resolvedTriggerIds = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
    }

    private class PendingOnlineTrigger
    {
        ...
        public string effectRef = "";
        public BattleSlotOwner sourceOwner;
        public string sourceSlotId = "";
        public string causedByEffectRef = "";
        public string causedByChainId = "";
        public bool mandatory = true;
        public bool cancelAllowed;
        public int queuedOrder;
    ...
    private void CopyOnlineEffectChainState(PendingOnlineSelection source, PendingOnlineSelection target)
    {
        if (source == null || target == null)
            return;

        target.chainId = source.chainId;
        target.originalViewerCost = source.originalViewerCost;
        target.viewerCostPaid = source.viewerCostPaid;
        target.actionUseCommitted = source.actionUseCommitted;
        target.costPaid = source.costPaid;

        foreach (PendingOnlineTrigger trigger in source.deferredTriggers)
        {
            if (trigger != null)
                target.deferredTriggers.Add(trigger);
        }

        foreach (string triggerId in source.resolvedTriggerIds)
        {
            if (!string.IsNullOrWhiteSpace(triggerId))
                target.resolvedTriggerIds.Add(triggerId);
        }
    }

    private void AddCommittedIdolActiveCostDeltas(
        ...
        string commitReason)
    {
        ...
        int cost = Mathf.Max(0, GetActiveCost(idolCard));
        int viewerBefore = GetViewersFromExternal(selection.requestedPlayer);
        int viewerAfter = Mathf.Max(0, viewerBefore - cost);

        selection.originalViewerCost = cost;
        selection.viewerCostPaid = true;
        selection.actionUseCommitted = true;
        selection.costPaid = true;
        ...
        {
            owner = selection.requestedPlayer,
            cardInstanceId = idolCard != null ? idolCard.cardInstanceId : selection.sourceCardInstanceId,
        ...
        Debug.Log(
            $"[OnlineEffectChain][CostCommit] chainId={selection.chainId}, " +
            $"actor={selection.requestedPlayer}, effectRef={selection.effectRef}, " +
            $"viewerBefore={viewerBefore}, viewerCost={cost}, viewerAfter={viewerAfter}, " +
            $"costPaid=True, reason={commitReason}");
    }
    ...
    public void HandleRejectedActionResultFromExternal(BattleActionResult result)
    {
            ...
            result.requestActionType == BattleActionType.SelectEffectChoice)
        {
            ...
            if (IsBattleBusy())
                SetBattleBusy(false, $"{result.requestActionType} rejected");
    ...
    private void FillBattleCountSnapshot(BattleActionResult result)
    {
        ...
        LogOnlineZoneAuthorityHostState(
            "[OnlineZoneAuthority][HostSnapshot]",
            result.actionSequence,
            result.requestActionType.ToString(),
            result.resolvedEffectRef);
    }

    private void LogOnlineZoneAuthorityHostState(
        string logKey,
        int actionSequence,
        string actionType,
        string effectRef)
    {
        ...
        Debug.Log(
            $"{logKey} actionSequence={actionSequence}, actionType={actionType}, effectRef={effectRef}, " +
            $"actor1HandActual={myPlayer?.hand?.Count ?? 0}, actor1HandIdsHash={HashCardInstanceIds(myPlayer?.hand)}, " +
            $"actor1DeckActual={myPlayer?.mainDeck?.Count ?? 0}, actor2HandActual={enemyPlayer?.hand?.Count ?? 0}, " +
            $"actor2HandIdsHash={HashCardInstanceIds(enemyPlayer?.hand)}, actor2DeckActual={enemyPlayer?.mainDeck?.Count ?? 0}");
    }

    private string HashCardInstanceIds(List<BaseCardData> cards)
    {
        if (cards == null || cards.Count == 0)
            return "0";
        ...
        foreach (BaseCardData card in cards)
        {
            ...
            ids.Add(!string.IsNullOrWhiteSpace(card.cardInstanceId) ? card.cardInstanceId : card.id);
        }
        ...
        unchecked
        {
            ...
            foreach (string id in ids)
                hash = hash * 31 + StringComparer.OrdinalIgnoreCase.GetHashCode(id ?? "");

            return hash.ToString("X8");
        }
    }

    public void LogOnlineZoneAuthorityBeforeResultFromExternal(BattleAction action)
    {
        if (action == null)
            return;

        LogOnlineZoneAuthorityHostState(
            "[OnlineZoneAuthority][HostBeforeResult]",
    ...
    private void EnqueueOnlineOnRestTriggersForChain(
        PendingOnlineSelection selection,
        BaseCardData restedCard,
        BattleSlotOwner restedOwner,
        string sourceSlotId,
        string restReason)
    {
        if (selection == null || restedCard == null)
            return;
        ...
        foreach (EffectData effect in character.effects)
        {
            ...
            {
                Debug.LogWarning(
                    $"[OnlineTriggerQueue][SkipUnsupported] chainId={selection.chainId}, " +
                    $"triggerType=OnRest, sourceCard={restedCard.cardInstanceId}, " +
                    $"effectRef={effectRef}, category={metadata.category}");
                continue;
            }

            int order = selection.deferredTriggers.Count + 1;
            PendingOnlineTrigger trigger = new PendingOnlineTrigger
            {
                triggerId = $"{selection.chainId}_trigger_{order}",
                triggerType = "OnRest",
                sourceCardInstanceId = restedCard.cardInstanceId,
                sourceCardId = restedCard.id,
                sourceCardName = restedCard.name,
                effectRef = effectRef,
                sourceOwner = restedOwner,
                sourceSlotId = sourceSlotId,
                causedByEffectRef = selection.effectRef,
                causedByChainId = selection.chainId,
                mandatory = true,
                cancelAllowed = false,
                queuedOrder = order,
            ...
            selection.deferredTriggers.Add(trigger);

            Debug.Log(
                $"[OnlineTriggerQueue][Enqueue] chainId={selection.chainId}, " +
                $"triggerType=OnRest, sourceCard={restedCard.cardInstanceId}/{restedCard.id}, " +
                $"effectRef={effectRef}, mandatory=True, reason={restReason}");
            Debug.Log(
                $"[OnlineTriggerQueue][Deferred] chainId={selection.chainId}, " +
                $"reason=RootEffectStillResolving, currentStep={selection.currentResolveStep}");
        }
    }
    ...
    private IEnumerator ApplyStartCollabFromResultRoutine(BattleActionResult result)
    {
        ...
        bool isEffectMoveCollab = string.Equals(
        ...
        bool usedImmediateFallback = false)
    {
        ...
        BattleActionType consumedActionType = string.Equals(
            result.resolvedEffectRef,
            "content.moveOwnCharToEmptyOrBattleIfTagged",
            StringComparison.OrdinalIgnoreCase)
        ...
        out string failReason)
    {
        ...
        if (!canResolve &&
            string.Equals(action.effectRef, "content.silenceCharacterCollabThisTurn", StringComparison.OrdinalIgnoreCase))
        {
            if (BuildOnlineFaceUpCharacterSlotsOnBoard().Count == 0)
            {
                failReason = "채팅 밴을 적용할 앞면 캐릭터가 없습니다.";
                return false;
            }
        ...
        if (!canResolve &&
            string.Equals(action.effectRef, "content.moveOwnCharToEmptyOrBattleIfTagged", StringComparison.OrdinalIgnoreCase))
        {
            if (card == null)
            {
                failReason = "효과 발동 콘텐츠 카드가 없습니다.";
                return false;
            }

            string tag = GetPrimaryEffectStringParam(card, action.effectRef, "tag", "");
            if (BuildOnlineTaggedMovableCharacterSlots(action.actor, tag).Count == 0)
            {
                failReason = $"이동시킬 {tag} 캐릭터가 없습니다.";
                return false;
            }
    ...
    public bool CanResolveUseIdolActiveOnlineFromExternal(BattleAction action, out string failReason)
    {
        failReason = "";

        if (action == null)
        {
            failReason = "UseIdolActive action is null.";
            return false;
        }

        if (action.actionType != BattleActionType.UseIdolActive)
        {
            failReason = $"UseIdolActive action type mismatch: {action.actionType}";
            return false;
        }
        ...
        {
            failReason = $"온라인 Selection Flow 1차는 아직 {effectRef} 아이돌 액티브를 처리하지 않습니다.";
            return false;
        }

        if (BuildOnlineControlledFaceUpCharacterSlots(action.actor).Count == 0)
        {
            failReason = "회복할 캐릭터가 없습니다.";
            return false;
        }
        ...
        int stepIndex)
    {
        ...
        PendingOnlineSelection selection = new PendingOnlineSelection
        {
            ...
            currentResolveStep = "SelectingBasicTabi"
        };
        if (parentSelection != null)
            CopyOnlineEffectChainState(parentSelection, selection);
        else
            AddCommittedIdolActiveCostDeltas(result, selection, idolCard, "TabiBasicInitialRequest");
        ...
        List<BattleFieldSlot> candidates)
    {
        ...
        PendingOnlineSelection selection = new PendingOnlineSelection
        {
            ...
            currentResolveStep = "SelectingEnhancedCost"
        };
        AddCommittedIdolActiveCostDeltas(result, selection, idolCard, "TabiEnhancedInitialRequest");
    ...
    public bool CanResolveUseCharacterActiveOnlineFromExternal(
        BattleAction action,
        out string failReason)
    {
        failReason = "";
        ...
        if (action == null)
        {
            failReason = "UseCharacterActive action is null.";
            return false;
        }

        if (action.actionType != BattleActionType.UseCharacterActive)
        {
            failReason = $"UseCharacterActive action type mismatch: {action.actionType}";
            return false;
        }

        BattleFieldSlot sourceSlot = FindFieldSlotBySlotId(action.sourceSlotId);
        if (sourceSlot == null)
        {
        ...
        if (!CanUseCharacterActiveForOwner(sourceSlot, action.actor, out failReason))
            return false;
        ...
        if (!IsCharacterActiveEffectRef(sourceSlot.characterCard, effectRef))
        {
            failReason = "선택한 캐릭터의 액티브 효과와 요청 effectRef가 일치하지 않습니다.";
            return false;
        }
    ...
    public BattleActionResult CreateUseCharacterActiveSelectionRequestResultFromExternal(BattleAction action)
    {
        BattleFieldSlot sourceSlot = action != null
            ? FindFieldSlotBySlotId(action.sourceSlotId)
            : null;
        ...
        BattleActionResult result = new BattleActionResult
        {
            ...
            isAccepted = true,
            message = "캐릭터 액티브 효과 대상 선택이 필요합니다.",
            currentTurnPlayer = action != null ? action.actor : BattleSlotOwner.My,
            turnCount = Mathf.Max(1, turnCount),
            nextPhase = BattlePhase.MainGame.ToString(),
            resolvedEffectRef = effectRef,
        ...
        PendingOnlineSelection selection = new PendingOnlineSelection
        {
            ...
            stepIndex = 1,
            chainId = requestId,
            resolveKind = "SelectTargetForCharacterActiveModifyTaggedOnBoard"
        };
    ...
    {
        BaseCardData contentCard = action != null
            ? FindHandCardByInstanceId(action.actor, action.cardInstanceId)
            : null;
        string effectRef = action != null && !string.IsNullOrWhiteSpace(action.effectRef)
            ? action.effectRef
            : effectManager != null ? effectManager.GetPrimaryEffectRefFromExternal(contentCard) : "";
        string tag = GetPrimaryEffectStringParam(contentCard, effectRef, "tag", "");
        bool isMoveTaggedEffect = string.Equals(
            effectRef,
            "content.moveOwnCharToEmptyOrBattleIfTagged",
            StringComparison.OrdinalIgnoreCase);
        List<BattleFieldSlot> candidates = isMoveTaggedEffect
            ? BuildOnlineTaggedMovableCharacterSlots(action != null ? action.actor : BattleSlotOwner.My, tag)
            : BuildOnlineFaceUpCharacterSlotsOnBoard();
        ...
        if (contentCard != null)
            result.affectedCardIds.Add(contentCard.cardInstanceId);
        else if (action != null)
            result.affectedCardIds.Add(action.cardInstanceId);
        ...
        PendingOnlineSelection selection = new PendingOnlineSelection
        {
            requestId = requestId,
            effectRef = effectRef,
            sourceCardInstanceId = contentCard != null ? contentCard.cardInstanceId : action != null ? action.cardInstanceId : "",
            requestedPlayer = result.actor,
            sourceActionSequence = result.actionSequence,
            sourceActionType = BattleActionType.UseContent,
            stepIndex = 1,
            chainId = requestId,
            resolveKind = isMoveTaggedEffect
                ? "SelectTaggedOwnCharacterForMove"
                : "SelectSilenceCharacterCollabThisTurnTarget"
        };
    ...
    public BattleActionResult CreateSelectEffectTargetResultFromExternal(BattleAction action)
    {
        BattleActionResult result = new BattleActionResult
        {
            actionSequence = action != null ? action.actionSequence : nextActionSequence++,
            actor = action != null ? action.actor : BattleSlotOwner.My,
            requestActionType = BattleActionType.SelectEffectTarget,
            currentTurnPlayer = action != null ? action.actor : BattleSlotOwner.My,
            turnCount = Mathf.Max(1, turnCount),
            nextPhase = BattlePhase.MainGame.ToString()
        };
        ...
        {
            return CreateMoveTaggedDestinationSelectionRequestResult(action, result, selection, targetSlot);
        }

        if (selection.sourceActionType == BattleActionType.UseContent &&
            string.Equals(selection.resolveKind, "SelectDestinationForSelectedCharacter", StringComparison.OrdinalIgnoreCase))
        {
            return CreateMoveTaggedCharacterFinalResult(action, result, selection, targetSlot);
        }

        if (selection.sourceActionType == BattleActionType.UseContent &&
            string.Equals(selection.effectRef, "content.silenceCharacterCollabThisTurn", StringComparison.OrdinalIgnoreCase))
        {
            return CreateSilenceCharacterCollabSelectionResult(action, result, selection, targetSlot);
        }
        ...
        {
            result.isAccepted = false;
            result.rejectReason = $"지원하지 않는 선택 요청입니다: {selection.sourceActionType}/{selection.effectRef}";
            result.message = result.rejectReason;
            return result;
        }
    ...
    private BattleActionResult CreateCharacterActiveModifyTaggedOnBoardSelectionResult(
        BattleAction action,
        BattleActionResult result,
        PendingOnlineSelection selection,
        BattleFieldSlot targetSlot)
    {
        BattleFieldSlot sourceSlot = FindSlotById(selection.selectedSourceSlotId);
        BaseCardData sourceCard = sourceSlot != null ? sourceSlot.characterCard : null;
        EffectData effect = FindEffectDataByRefForBattleManager(sourceCard, selection.effectRef);
        int cost = GetActiveCost(sourceCard);
        int viewerBefore = GetViewersFromExternal(selection.requestedPlayer);
        int viewerAfter = Mathf.Max(0, viewerBefore - cost);
        ...
        int maxHpDelta = GetEffectIntParamForBattleManager(effect, "hpMaxDelta", 0);
        int hpDelta = GetEffectIntParamForBattleManager(effect, "hp", 0);
        int tensionDelta = GetEffectIntParamForBattleManager(effect, "tensionDelta", 0);
        if (tensionDelta == 0)
            tensionDelta = GetEffectIntParamForBattleManager(effect, "tension", 0);

        maxHpDelta = ApplyNegativeAmountInvertForEffectSource(selection.requestedPlayer, sourceCard, maxHpDelta);
        hpDelta = ApplyNegativeAmountInvertForEffectSource(selection.requestedPlayer, sourceCard, hpDelta);
        tensionDelta = ApplyNegativeAmountInvertForEffectSource(selection.requestedPlayer, sourceCard, tensionDelta);

        int maxHpAfter = Mathf.Max(1, maxHpBefore + maxHpDelta);
        int hpAfter = Mathf.Clamp(hpBefore + hpDelta, 0, maxHpAfter);
        int tensionAfter = Mathf.Max(0, tensionBefore + tensionDelta);
        ...
        result.effectApplied = true;
        result.paidViewerCost = cost;
        result.affectedSlotIds.Add(sourceSlot != null ? sourceSlot.GetSlotId() : "");
        result.affectedSlotIds.Add(targetSlot.GetSlotId());
        ...
        result.messageDeltas.Add(new MessageDelta
        {
            ...
            messageText = $"{(sourceCard != null ? sourceCard.name : "캐릭터")}의 액티브 효과로 {(targetSlot.characterCard != null ? targetSlot.characterCard.name : "선택 캐릭터")}의 스탯을 변경했습니다.",
        ...
        Debug.Log(
            $"[OnlineSelection] Host resolved character active modifyTaggedOnBoard. " +
            $"requestId={action.selectionRequestId}, source={sourceSlot?.GetSlotId()}, " +
        ...
        BattleFieldSlot targetSlot)
    {
        ...
        result.isAccepted = true;
        result.message = "채팅 밴 효과가 적용되었습니다.";
        result.resolvedEffectRef = selection.effectRef;
        result.effectApplied = true;
        result.paidViewerCost = cost;
        result.affectedSlotIds.Add(targetSlot.GetSlotId());
        ...
        result.messageDeltas.Add(new MessageDelta
        {
            audience = "Public",
            messageKey = selection.effectRef,
            messageText = $"{(targetSlot.characterCard != null ? targetSlot.characterCard.name : "선택 캐릭터")}은 이번 턴 합방 효과를 사용할 수 없습니다.",
            relatedCardId = targetSlot.characterCard != null ? targetSlot.characterCard.id : "",
            relatedInstanceId = targetSlot.characterCard != null ? targetSlot.characterCard.cardInstanceId : ""
        });
        ...
        Debug.Log(
            $"[OnlineSelection] Host resolved silence selection. requestId={action.selectionRequestId}, " +
            $"effectRef={selection.effectRef}, target={targetSlot.GetSlotId()}, untilTurn={turnCount}, cost={cost}");
        return result;
    }

    private BattleActionResult CreateMoveTaggedDestinationSelectionRequestResult(
        BattleAction action,
        BattleActionResult result,
        PendingOnlineSelection selection,
        BattleFieldSlot sourceSlot)
    {
        List<BattleFieldSlot> destinations = BuildOnlineEffectMoveDestinationSlots(
            selection.requestedPlayer,
            sourceSlot);
        if (destinations.Count == 0)
        {
            result.isAccepted = false;
            result.rejectReason = "이동 가능한 위치가 없습니다.";
            result.message = result.rejectReason;
            return result;
        }
        ...
        PendingOnlineSelection nextSelection = new PendingOnlineSelection
        {
            ...
            effectRef = selection.effectRef,
            sourceCardInstanceId = selection.sourceCardInstanceId,
            selectedSourceCharacterInstanceId = sourceSlot.characterCard != null ? sourceSlot.characterCard.cardInstanceId : "",
            selectedSourceSlotId = sourceSlot.GetSlotId(),
            requestedPlayer = selection.requestedPlayer,
            sourceActionSequence = selection.sourceActionSequence,
            sourceActionType = BattleActionType.UseContent,
            stepIndex = 2,
            resolveKind = "SelectDestinationForSelectedCharacter"
        };

        result.isAccepted = true;
        result.message = "이동할 위치 선택이 필요합니다.";
        result.resolvedEffectRef = selection.effectRef;
        ...
        SelectionRequestDelta request = new SelectionRequestDelta
        {
            ...
            cancelPolicy = "DisallowCancel",
            nextActionType = BattleActionType.SelectEffectTarget.ToString(),
            promptText = "이동할 위치를 골라주세요."
        };

        foreach (BattleFieldSlot slot in destinations)
        {
            if (slot == null)
                continue;
        ...
        BattleFieldSlot boongSlot)
    {
        ...
        selection.currentResolveStep = "SelectingTabi";
        EnqueueOnlineOnRestTriggersForChain(
            selection,
            boongCard,
            boongOwner,
            boongSlotId,
        ...
        PendingOnlineSelection nextSelection = new PendingOnlineSelection
        {
            ...
            costPaid = true,
            currentResolveStep = "SelectingTabi"
        };
        CopyOnlineEffectChainState(selection, nextSelection);
    ...
    private BattleActionResult CreateMoveTaggedCharacterFinalResult(
        BattleAction action,
        BattleActionResult result,
        PendingOnlineSelection selection,
        BattleFieldSlot targetSlot)
    {
        ...
        if (sourceSlot == null || !sourceSlot.HasCharacter || sourceSlot.characterCard == null)
        {
            result.isAccepted = false;
            result.rejectReason = "이동할 캐릭터가 필드에 없습니다.";
            result.message = result.rejectReason;
            return result;
        }
        ...
        if (targetSlot != null &&
            ...
            targetSlot.characterOwner != selection.requestedPlayer)
        {
            return CreateMoveTaggedCharacterCollabResult(
                action,
                selection,
                sourceSlot,
                targetSlot,
        ...
        result.isAccepted = true;
        result.message = "캐릭터 이동 효과가 적용되었습니다.";
        result.resolvedEffectRef = selection.effectRef;
        result.effectApplied = true;
        result.paidViewerCost = cost;
        result.faceDown = sourceSlot.isCharacterFaceDown;
        result.characterOwner = sourceSlot.characterOwner;
        result.characterCurrentHp = sourceSlot.currentCharacterHp;
        result.characterCurrentMaxHp = sourceSlot.currentCharacterMaxHp;
        result.characterCurrentTension = sourceSlot.currentCharacterTension;
        result.characterMovedThisTurn = sourceSlot.characterMovedThisTurn;
        result.characterActiveUsedThisTurn = sourceSlot.characterActiveUsedThisTurn;

        if (contentCard != null)
            result.affectedCardIds.Add(contentCard.cardInstanceId);
        if (sourceSlot.characterCard != null)
        {
        ...
        result.messageDeltas.Add(new MessageDelta
        {
            audience = "Public",
            messageKey = selection.effectRef,
            messageText = $"{(sourceSlot.characterCard != null ? sourceSlot.characterCard.name : "선택 캐릭터")}를 이동했습니다.",
            relatedCardId = sourceSlot.characterCard != null ? sourceSlot.characterCard.id : "",
        ...
        Debug.Log(
            $"[OnlineSelection] Host resolved move effect. requestId={action.selectionRequestId}, " +
            $"source={sourceSlot.GetSlotId()}, target={targetSlot.GetSlotId()}, cost={cost}");
        return result;
    }

    private BattleActionResult CreateMoveTaggedCharacterCollabResult(
        BattleAction action,
        PendingOnlineSelection selection,
        BattleFieldSlot sourceSlot,
        BattleFieldSlot targetSlot,
        ...
        int viewerAfter)
    {
        ...
        {
            actionSequence = action != null ? action.actionSequence : nextActionSequence++,
            actor = selection.requestedPlayer,
            actionType = BattleActionType.StartCollab,
            cardInstanceId = sourceSlot.characterCard != null ? sourceSlot.characterCard.cardInstanceId : "",
            sourceSlotId = sourceSlot.GetSlotId(),
            targetSlotId = targetSlot.GetSlotId()
        };

        BattleActionResult collabResult = CreateStartCollabResultFromExternal(collabAction);
        collabResult.resolvedEffectRef = selection.effectRef;
        collabResult.effectApplied = true;
        collabResult.effectClassification = "EffectMoveCollab";
        collabResult.paidViewerCost = cost;

        if (cost > 0)
        {
            collabResult.viewerDeltas.Add(new ViewerDelta
            {
                owner = selection.requestedPlayer,
                before = viewerBefore,
                after = viewerAfter,
        ...
        if (contentCard != null)
        {
            collabResult.cardZoneMoveDeltas.Add(new CardZoneMoveDelta
            {
                cardInstanceId = contentCard.cardInstanceId,
                cardId = contentCard.id,
                owner = selection.requestedPlayer,
            ...
            collabResult.affectedCardIds.Add(contentCard.cardInstanceId);
        ...
        collabResult.messageDeltas.Add(new MessageDelta
        {
            audience = "Public",
            messageKey = selection.effectRef,
            messageText = $"{(sourceSlot.characterCard != null ? sourceSlot.characterCard.name : "선택 캐릭터")}가 효과로 합방을 시작했습니다.",
            relatedCardId = sourceSlot.characterCard != null ? sourceSlot.characterCard.id : "",
            relatedInstanceId = sourceSlot.characterCard != null ? sourceSlot.characterCard.cardInstanceId : ""
        });
    ...
    private void ProjectUseContentCostAndHandIntoSnapshot(
        BattleActionResult result,
        BattleSlotOwner owner,
        int cost)
    {
    ...
    private List<BattleFieldSlot> BuildOnlineControlledFaceUpCharacterSlots(BattleSlotOwner owner)
    {
        ...
        AddOnlineControlledFaceUpCharacterSlots(candidates, myBattleSlots, owner);
        AddOnlineControlledFaceUpCharacterSlots(candidates, enemyBattleSlots, owner);
        return candidates;
    }
        ...
        BattleSlotOwner actingOwner)
    {
        ...
        foreach (BattleFieldSlot slot in slots)
        {
            if (!IsOnlineModifyTaggedOnBoardTargetSlot(slot, tag, targetOwner, targetScope, actingOwner))
                continue;
    ...
    private bool IsOnlineModifyTaggedOnBoardTargetSlot(
        BattleFieldSlot slot,
        string tag,
        string targetOwner,
        string targetScope,
    ...
    {
        List<BattleFieldSlot> candidates = new List<BattleFieldSlot>();
        AddOnlineTaggedMovableCharacterSlots(candidates, myBattleSlots, owner, tag);
        AddOnlineTaggedMovableCharacterSlots(candidates, enemyBattleSlots, owner, tag);
        return candidates;
    }

    private void AddOnlineTaggedMovableCharacterSlots(
        List<BattleFieldSlot> candidates,
        List<BattleFieldSlot> slots,
        BattleSlotOwner owner,
        string tag)
    {
        ...
        foreach (BattleFieldSlot slot in slots)
        {
            if (!IsOnlineTaggedMoveSourceSlot(slot, owner, tag))
                continue;

            if (BuildOnlineEffectMoveDestinationSlots(owner, slot).Count == 0)
                continue;
    ...
    private bool IsOnlineTaggedMoveSourceSlot(
        BattleFieldSlot slot,
        BattleSlotOwner owner,
        string tag)
    {
    ...
    private List<BattleFieldSlot> BuildOnlineEmptyEffectMoveDestinationSlots(BattleFieldSlot sourceSlot)
    {
        List<BattleFieldSlot> emptyDestinations = new List<BattleFieldSlot>();

        if (sourceSlot == null || movementManager == null)
            return emptyDestinations;

        List<BattleFieldSlot> candidates = movementManager.BuildMoveCandidatesForEffectFromExternal(
            sourceSlot.characterOwner,
            sourceSlot);
        if (candidates == null)
            return emptyDestinations;
        ...
        foreach (BattleFieldSlot slot in candidates)
        {
            ...
            if (movementManager.CanMoveCharacterByEffectForOwnerFromExternal(
                sourceSlot.characterOwner,
                sourceSlot,
                slot,
                out _))
            {
                emptyDestinations.Add(slot);
            }
        }
    ...
    private List<BattleFieldSlot> BuildOnlineEffectMoveDestinationSlots(
        BattleSlotOwner actingOwner,
        BattleFieldSlot sourceSlot)
    {
        List<BattleFieldSlot> destinations = new List<BattleFieldSlot>();
        ...
        foreach (BattleFieldSlot slot in candidates)
        {
            if (slot == null || destinations.Contains(slot))
                continue;
            ...
            if (slot.characterOwner == actingOwner)
                continue;

            if (movementManager.CanStartCollaborationByEffectForOwnerFromExternal(
                actingOwner,
                sourceSlot,
                slot,
                out _))
    ...
    private string GetEffectMoveTargetKind(BattleFieldSlot sourceSlot, BattleFieldSlot targetSlot)
    {
        if (targetSlot != null &&
            targetSlot.HasCharacter &&
            sourceSlot != null &&
        ...
        {
            return "CollabTargetSlot";
        }

        return "EmptyMoveSlot";
    }

    private void AddOnlineFaceUpCharacterSlots(
        List<BattleFieldSlot> candidates,
        List<BattleFieldSlot> slots)
    {
        if (candidates == null || slots == null)
    ...
    private void AddOnlineControlledFaceUpCharacterSlots(
        List<BattleFieldSlot> candidates,
        List<BattleFieldSlot> slots,
        BattleSlotOwner owner)
    {
    ...
    private SelectionRequestTarget BuildSelectionRequestTarget(BattleFieldSlot slot, string targetKind = "")
    {
        string slotId = slot != null ? slot.GetSlotId() : "";
        bool hasPublicCardInfo = slot != null &&
            slot.characterCard != null &&
            !slot.isCharacterFaceDown;

        return new SelectionRequestTarget
        {
            targetKind = targetKind,
            slotId = slotId,
            slotOwner = slot != null ? slot.owner : BattleSlotOwner.My,
            slotIndex = ParseSlotIndex(slotId),
            row = ParseSlotRow(slotId),
            characterInstanceId = hasPublicCardInfo ? slot.characterCard.cardInstanceId : "",
            targetCardInstanceId = hasPublicCardInfo ? slot.characterCard.cardInstanceId : "",
            cardId = hasPublicCardInfo ? slot.characterCard.id : "",
            displayName = hasPublicCardInfo ? slot.characterCard.name : "",
            isPublicCardInfo = hasPublicCardInfo,
            isFaceDown = slot != null && slot.isCharacterFaceDown
        };
    }

    private bool TryValidatePendingOnlineSelection(
        BattleAction action,
        ...
        out string failReason)
    {
        ...
        if (action == null)
        {
            failReason = "SelectEffectTarget action is null.";
            return false;
        }
        ...
        if (selection.sourceActionType == BattleActionType.UseIdolActive)
        {
            bool isCommittedTabiChain = string.Equals(
                selection.effectRef,
                "idol.active.fetchTabiOrRestBoongAndFetchBoth",
                StringComparison.OrdinalIgnoreCase);
            Debug.Log(
                $"[OnlineEffectChain][FollowUpValidate] chainId={selection.chainId}, " +
                $"requestId={action.selectionRequestId}, step={selection.currentResolveStep}, " +
                $"viewerCostRevalidated={!isCommittedTabiChain}, costPaid={selection.viewerCostPaid}");

            if (!targetSlot.HasCharacter ||
                targetSlot.characterCard == null ||
                targetSlot.isCharacterFaceDown)
            {
                failReason = "선택한 효과 대상 슬롯 상태가 유효하지 않습니다.";
                return false;
            }
            ...
            if (isCommittedTabiChain)
            {
                if (!selection.viewerCostPaid || !selection.actionUseCommitted)
                {
                    failReason = "아이돌 액티브 비용 커밋 상태가 유효하지 않습니다.";
                    return false;
                }
            ...
            string.Equals(selection.resolveKind, "SelectTargetForCharacterActiveModifyTaggedOnBoard", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (!IsOnlineModifyTaggedOnBoardTargetSlot(
                    targetSlot,
                    tag,
                    targetOwner,
                    targetScope,
    ...
    public BattleActionResult CreateSelectEffectChoiceResultFromExternal(BattleAction action)
    {
        ...
        if (TryGetPendingMandatoryTriggerSelection(action, out PendingOnlineSelection triggerSelection))
            return CreateMandatoryTriggerConfirmedResult(action, result, triggerSelection);
    ...
    private bool TryGetPendingMandatoryTriggerSelection(
        BattleAction action,
        out PendingOnlineSelection selection)
    {
        selection = null;
        ...
        return string.Equals(selection.resolveKind, "MandatoryTriggerConfirm", StringComparison.OrdinalIgnoreCase);
    }

    private BattleActionResult CreateMandatoryTriggerConfirmedResult(
        BattleAction action,
        BattleActionResult result,
        PendingOnlineSelection selection)
    {
        if (!TryValidateMandatoryTriggerConfirm(
                action,
                selection,
                out PendingOnlineTrigger trigger,
                out string failReason))
        {
            RejectPendingOnlineSelection(action, failReason);
            result.isAccepted = false;
        ...
        OnlineEffectResolver resolver = new OnlineEffectResolver(this);
        OnlineEffectResolveResult triggerResult =
            resolver.ResolveSimpleEffectForHost(trigger.effectRef, trigger.sourceCard, trigger.effectData, trigger.sourceOwner);

        if (triggerResult == null || !triggerResult.success)
        {
            string reason = triggerResult?.rejectReason ?? "트리거 효과 해결에 실패했습니다.";
            RejectPendingOnlineSelection(action, reason);
            result.isAccepted = false;
        ...
        result.requestActionType = BattleActionType.SelectEffectChoice;
        result.resolvedEffectRef = trigger.effectRef;
        result.effectApplied = true;
        result.requiresFollowUpAction = false;
        result.effectClassification = "DeferredTriggerResolved";
        result.message = "강제 트리거 효과가 적용되었습니다.";

        MergeOnlineEffectResolveResult(result, triggerResult);
        selection.resolvedTriggerIds.Add(trigger.triggerId);

        Debug.Log(
            $"[OnlineTriggerFlow][Confirm] triggerId={trigger.triggerId}, " +
            $"actor={action.actor}, effectRef={trigger.effectRef}, mandatory={trigger.mandatory}");

        PendingOnlineTrigger nextTrigger = FindNextUnresolvedTrigger(selection);
        if (nextTrigger != null)
        {
            FillBattleCountSnapshot(result);
            ProjectViewerDeltasIntoSnapshot(result);
            Debug.Log(
                $"[OnlineTriggerFlow][Resolved] triggerId={trigger.triggerId}, " +
                $"viewerDelta={SumViewerDeltaAmounts(result)}, remainingTriggers=1");
            return CreateNextDeferredTriggerRequestResult(action, result, selection, "");
        }
        ...
        Debug.Log(
            $"[OnlineTriggerFlow][Resolved] triggerId={trigger.triggerId}, " +
            $"viewerDelta={SumViewerDeltaAmounts(result)}, remainingTriggers=0");
        Debug.Log(
            $"[OnlineEffectChain][Complete] chainId={selection.chainId}, " +
            "costPaid=True, rootResolved=True, triggersResolved=True");
        return result;
    }

    private bool TryValidateMandatoryTriggerConfirm(
        BattleAction action,
        PendingOnlineSelection selection,
        out PendingOnlineTrigger trigger,
        out string failReason)
    {
        ...
        {
            failReason = "강제 트리거 확인 요청이 유효하지 않습니다.";
            return false;
        }

        if (resolvedOnlineSelectionRequestIds.Contains(action.selectionRequestId))
        {
            failReason = "이미 처리된 트리거 요청입니다.";
            return false;
        }

        if (action.actor != selection.requestedPlayer)
        {
            failReason = "트리거 확인 대상 플레이어와 응답 플레이어가 일치하지 않습니다.";
            return false;
        }

        if (!string.Equals(action.choiceId, "mandatoryTrigger", StringComparison.OrdinalIgnoreCase) ||
            !string.Equals(action.choiceValue, "confirm", StringComparison.OrdinalIgnoreCase))
        {
            failReason = "강제 트리거는 취소할 수 없습니다.";
            return false;
        }

        trigger = FindNextUnresolvedTrigger(selection);
        if (trigger == null)
        {
            failReason = "해결할 강제 트리거가 없습니다.";
            return false;
        }

        if (!string.Equals(action.effectRef, trigger.effectRef, StringComparison.OrdinalIgnoreCase))
        {
            failReason = "대기 중인 트리거 효과와 응답 effectRef가 일치하지 않습니다.";
            return false;
        }

        if (trigger.sourceCard == null || trigger.effectData == null)
        {
            failReason = "트리거 원본 카드 또는 효과 정보가 유효하지 않습니다.";
            return false;
        }
    ...
    private int SumViewerDeltaAmounts(BattleActionResult result)
    {
        if (result == null || result.viewerDeltas == null)
            return 0;
        ...
        out string failReason)
    {
        ...
        Debug.Log(
            $"[OnlineEffectChain][FollowUpValidate] chainId={selection.chainId}, " +
            $"requestId={action.selectionRequestId}, step={selection.currentResolveStep}, " +
            "viewerCostRevalidated=False, costPaid=True");
        return true;
    }
    ...
    private BattleActionResult CreatePrivateDeckToHandFinalResult(
        ...
        string ownerMessage)
    {
        bool chainCostAlreadyPaid = selection.viewerCostPaid && selection.actionUseCommitted;
        int costToApply = chainCostAlreadyPaid ? 0 : cost;
        int viewerBefore = GetViewersFromExternal(selection.requestedPlayer);
        int viewerAfter = Mathf.Max(0, viewerBefore - costToApply);
        ...
        result.paidViewerCost = costToApply;
        ...
        FillBattleCountSnapshot(result);
        ProjectPrivateDeckToHandIntoSnapshot(result, selection.requestedPlayer, costToApply);

        if (selection.deferredTriggers.Count > 0)
        {
            selection.currentResolveStep = "RootEffectResolved";
            Debug.Log(
                $"[OnlineEffectChain][RootResolved] chainId={selection.chainId}, " +
                $"effectRef={selection.effectRef}, drawnTabi={selection.selectedTabiCardInstanceId}, " +
                $"drawnBoong={selectedCard?.cardInstanceId ?? ""}, " +
                $"deferredTriggerCount={selection.deferredTriggers.Count}");
            return CreateNextDeferredTriggerRequestResult(
                action,
                result,
                selection,
        ...
        Debug.Log(
            ...
            $"effectRef={selection.effectRef}, cost={costToApply}");
        return result;
    }

    private BattleActionResult CreateNextDeferredTriggerRequestResult(
        BattleAction action,
        BattleActionResult result,
        PendingOnlineSelection selection,
        string finalDrawnCardInstanceId)
    {
        PendingOnlineTrigger trigger = FindNextUnresolvedTrigger(selection);
        if (trigger == null)
        {
            CompletePendingOnlineSelection(action.selectionRequestId, PendingCompleteSuccess);
            result.requiresFollowUpAction = false;
        ...
        string requestId = $"{selection.chainId}_trigger_{trigger.queuedOrder}_confirm";
        PendingOnlineSelection triggerSelection = new PendingOnlineSelection
        {
            requestId = requestId,
            parentRequestId = selection.requestId,
            chainId = selection.chainId,
            effectRef = trigger.effectRef,
            sourceCardInstanceId = trigger.sourceCardInstanceId,
            requestedPlayer = trigger.sourceOwner,
            sourceActionSequence = selection.sourceActionSequence,
            sourceActionType = BattleActionType.SelectEffectChoice,
            stepIndex = selection.stepIndex + 1,
            resolveKind = "MandatoryTriggerConfirm",
            currentResolveStep = "ResolvingDeferredTriggers",
            paidBoongCardInstanceId = selection.paidBoongCardInstanceId,
            paidBoongFromSlotId = selection.paidBoongFromSlotId,
            selectedTabiCardInstanceId = selection.selectedTabiCardInstanceId
        };
        CopyOnlineEffectChainState(selection, triggerSelection);
        triggerSelection.currentResolveStep = "ResolvingDeferredTriggers";
        ...
        SelectionRequestDelta request = new SelectionRequestDelta
        {
            ...
            sourceActionSequence = selection.sourceActionSequence,
            sourceCardInstanceId = trigger.sourceCardInstanceId,
            sourceSlotId = trigger.sourceSlotId,
            requestedPlayer = trigger.sourceOwner,
            requestingPlayer = trigger.sourceOwner,
            sourceActionType = BattleActionType.SelectEffectChoice.ToString(),
            sourceEffectRef = trigger.effectRef,
            choiceType = "ConfirmOnly",
            selectionType = "MandatoryTriggerConfirm",
            minSelect = 0,
            maxSelect = 0,
            cancelPolicy = "DisallowCancel",
            nextActionType = BattleActionType.SelectEffectChoice.ToString(),
            promptText = BuildMandatoryTriggerPrompt(trigger),
        ...
        request.candidatePublicIds.Add(trigger.sourceCardInstanceId);

        result.requiresFollowUpAction = true;
        result.effectApplied = false;
        result.effectClassification = OnlineEffectProgressStatus.NeedsSelection.ToString();
        result.selectionRequests.Clear();
        result.selectionRequests.Add(request);

        RegisterPendingOnlineSelection(triggerSelection);

        Debug.Log(
            $"[OnlineTriggerQueue][BeginResolve] chainId={selection.chainId}, " +
            $"triggerId={trigger.triggerId}, effectRef={trigger.effectRef}");
        return result;
    }

    private PendingOnlineTrigger FindNextUnresolvedTrigger(PendingOnlineSelection selection)
    {
        if (selection == null || selection.deferredTriggers == null)
            return null;

        foreach (PendingOnlineTrigger trigger in selection.deferredTriggers)
        {
            if (trigger == null || string.IsNullOrWhiteSpace(trigger.triggerId))
                continue;

            if (!selection.resolvedTriggerIds.Contains(trigger.triggerId))
                return trigger;
        }
    ...
    private string BuildMandatoryTriggerPrompt(PendingOnlineTrigger trigger)
    {
        return "강제 발동 효과를 처리합니다.";
    }
        ...
        BaseCardData idolCard)
    {
        ...
        PendingOnlineSelection nextSelection = new PendingOnlineSelection
        {
            ...
            currentResolveStep = "SelectingBoong"
        };
        CopyOnlineEffectChainState(selection, nextSelection);
        nextSelection.currentResolveStep = "SelectingBoong";
        ...
        string ownerMessage)
    {
        ...
        int fromDeckIndex = FindMainDeckCardIndexByInstanceId(
            selection.requestedPlayer,
            selectedCard.cardInstanceId);
        bool hostMutationApplied = ApplyAuthoritativePrivateDeckToHandMutation(
            selection.requestedPlayer,
            selectedCard,
            selection.effectRef,
            selection.chainId,
        ...
        result.cardDrawDeltas.Add(new CardDrawDelta
        {
            ...
            fromDeckIndex = fromDeckIndex,
            toHand = true,
            visibleToOwnerOnly = true,
            publicCardIdForOpponent = selectedCard.id,
            acquisitionVisibility = CardAcquisitionVisibility.RevealSelected.ToString()
        });

        result.cardRevealDeltas.Add(new CardRevealDelta
        {
            revealId = $"{selection.chainId}_{selectedCard.cardInstanceId}_reveal",
            actionSequence = result.actionSequence,
            rootActionSequence = selection.sourceActionSequence,
            chainId = selection.chainId,
            sourceEffectRef = selection.effectRef,
            sourceCardInstanceId = selection.sourceCardInstanceId,
            actor = selection.requestedPlayer,
            revealedCardId = selectedCard.id,
            revealedCardInstanceId = selectedCard.cardInstanceId,
            revealedCardOwner = selection.requestedPlayer,
            revealReason = CardRevealReason.SearchToHand.ToString(),
            destinationZone = "Hand",
            visibility = CardAcquisitionVisibility.RevealSelected.ToString(),
            publicMessageKey = "SearchToHandRevealSelected",
        ...
        Debug.Log(
            $"[OnlinePrivateDeckToHand][HostMutation] actionSequence={result.actionSequence}, " +
            $"chainId={selection.chainId}, effectRef={selection.effectRef}, owner={selection.requestedPlayer}, " +
            $"card={selectedCard.cardInstanceId}, fromDeckIndex={fromDeckIndex}, applied={hostMutationApplied}");
        Debug.Log(
            $"[CardAcquisition][Classify] effectRef={selection.effectRef}, " +
            $"visibility={CardAcquisitionVisibility.RevealSelected}, sourceZone=Deck, destinationZone=Hand");
        Debug.Log(
            $"[CardReveal][PublicSearch] actionSequence={result.actionSequence}, chainId={selection.chainId}, " +
            $"actor={selection.requestedPlayer}, effectRef={selection.effectRef}, " +
            $"cardId={selectedCard.id}, cardInstanceId={selectedCard.cardInstanceId}, reason=SearchToHand");
    }

    private bool ApplyAuthoritativePrivateDeckToHandMutation(
        BattleSlotOwner owner,
        BaseCardData selectedCard,
        string effectRef,
        string chainId,
        int actionSequence)
    {
        ...
        if (player == null || player.mainDeck == null || player.hand == null || selectedCard == null)
            return false;

        bool alreadyInHand = ContainsCardInstance(player.hand, selectedCard.cardInstanceId);
        bool removedFromDeck = player.mainDeck.Remove(selectedCard);

        if (!removedFromDeck && alreadyInHand)
        {
            Debug.Log(
                $"[OnlinePrivateDeckToHand][HostMutationSkip] actionSequence={actionSequence}, " +
                $"chainId={chainId}, effectRef={effectRef}, owner={owner}, " +
                $"card={selectedCard.cardInstanceId}, reason=AlreadyInHand");
            return true;
        }
        ...
        {
            Debug.LogWarning(
                $"[OnlineZoneDesync] actor={owner}, actionSequence={actionSequence}, zone=Deck, " +
                $"actual={player.mainDeck.Count}, authoritativeCount={player.mainDeck.Count}, " +
                $"reason=SelectedCardMissingBeforeHostMutation, card={selectedCard.cardInstanceId}, effectRef={effectRef}");
            return false;
        }
        ...
        LogOnlineZoneAuthorityHostState(
            "[OnlineZoneAuthority][HostAfterMutation]",
            actionSequence,
            "PrivateDeckToHand",
            effectRef);
        return true;
    ...
    private bool ContainsCardInstance(List<BaseCardData> cards, string cardInstanceId)
    {
    ...
    private int ParseSlotIndex(string slotId)
    {
        ...
        return int.TryParse(parts[2], out int index) ? index : -1;
    }

    private string ParseSlotRow(string slotId)
    {
        if (string.IsNullOrWhiteSpace(slotId))
            return "";
    ...
    private string GetPrimaryEffectStringParam(
        BaseCardData card,
        string effectRef,
        string key,
        string defaultValue)
    {
        EffectData effect = FindEffectDataByRefForBattleManager(card, effectRef);
        return GetEffectStringParamForBattleManager(effect, key, defaultValue);
    }

    private EffectData FindEffectDataByRefForBattleManager(BaseCardData card, string effectRef)
    {
        if (card == null || string.IsNullOrWhiteSpace(effectRef))
            return null;

        if (card is ContentCardData content)
            return FindEffectDataByRefForBattleManager(content.effects, effectRef);

        if (card is CharacterCardData character)
            return FindEffectDataByRefForBattleManager(character.effects, effectRef);

        if (card is BroadcastCardData broadcast)
            return FindEffectDataByRefForBattleManager(broadcast.effects, effectRef);

        if (card is IdolCardData idol)
        {
            EffectData active = FindEffectDataByRefForBattleManager(idol.active, effectRef);
            return active ?? FindEffectDataByRefForBattleManager(idol.passive, effectRef);
        }

        return null;
    }

    private EffectData FindEffectDataByRefForBattleManager(EffectData[] effects, string effectRef)
    {
        if (effects == null)
            return null;
        ...
        foreach (EffectData effect in effects)
        {
            ...
            if (string.Equals(GetEffectRefForBattleManager(effect), effectRef, StringComparison.OrdinalIgnoreCase))
                return effect;
        }
    ...
    public void ApplyOnlineSelectionRequestFromResult(BattleActionResult result)
    {
        ...
        {
            ApplyEffectDeltasFromResult(result);
            if (HasIdolActiveActionStateDelta(result))
            {
                MarkOnlinePlayerUsedActionFromExternal(
                    result.actor,
                    BattleActionType.UseIdolActive);
        ...
        if (request.requestedPlayer != BattleSlotOwner.My)
        {
            ...
            SetSystemMessage(
                string.Equals(request.selectionType, "MandatoryTriggerConfirm", StringComparison.OrdinalIgnoreCase)
                    ? "상대가 강제 효과를 확인 중입니다."
                    : "상대가 효과 대상을 선택 중입니다.");
            return;
        }

        if (string.Equals(request.selectionType, "MandatoryTriggerConfirm", StringComparison.OrdinalIgnoreCase) ||
            string.Equals(request.choiceType, "ConfirmOnly", StringComparison.OrdinalIgnoreCase))
        {
            ApplyMandatoryTriggerConfirmRequest(request);
            return;
        }
        ...
        {
            foreach (SelectionRequestTarget target in request.candidateTargets)
            {
                if (target == null || string.IsNullOrWhiteSpace(target.slotId))
                    continue;

                BattleFieldSlot slot = FindSlotById(target.slotId);
                if (slot != null)
                    candidates.Add(slot);
            }
        }
        ...
        if (cardQuestionPanel != null)
            cardQuestionPanel.CloseRevealNotificationForHigherPriorityUI(request.selectionType);
    ...
    private bool HasIdolActiveActionStateDelta(BattleActionResult result)
    {
        if (result == null || result.actionStateDeltas == null)
            return false;

        foreach (ActionStateDelta delta in result.actionStateDeltas)
        {
            if (delta != null &&
                string.Equals(delta.actionStateType, "IdolActiveUsedThisTurn", StringComparison.OrdinalIgnoreCase) &&
                delta.after)
            {
                return true;
            }
    ...
    private void ApplyMandatoryTriggerConfirmRequest(SelectionRequestDelta request)
    {
        if (request == null)
            return;
        ...
        if (cardQuestionPanel == null)
        {
            ...
            SetBattleBusy(false, "MandatoryTriggerConfirm missing panel");
            return;
        }

        BaseCardData sourceCard = FindCardByInstanceIdFromAnyKnownZone(
            request.requestedPlayer,
            request.sourceCardInstanceId);
        if (sourceCard == null)
        {
            sourceCard = new BaseCardData
            {
                id = request.sourceCardId,
                name = string.IsNullOrWhiteSpace(request.sourceCardName) ? "트리거 카드" : request.sourceCardName,
                kind = "Character",
                cardInstanceId = request.sourceCardInstanceId
            };
        }

        bool opened = cardQuestionPanel.TryShowCardsForConfirmation(
            string.IsNullOrWhiteSpace(request.promptText) ? "강제 발동 효과를 처리합니다." : request.promptText,
            new List<BaseCardData> { sourceCard },
            () => RequestSelectEffectChoiceActionFromExternal(
                request.sourceEffectRef,
                "mandatoryTrigger",
                "confirm"),
            false);
        ...
        {
            activeOnlineSelectionRequest = null;
            SetBattleBusy(false, "MandatoryTriggerConfirm open failed");
            return;
        }

        SetBattleBusy(true, "MandatoryTriggerConfirm");
        SetSystemMessage("강제 효과 확인이 필요합니다.");
    }
    ...
    public void ApplySelectEffectTargetFromResult(BattleActionResult result)
    {
        ...
        if (string.Equals(
            ...
            StringComparison.OrdinalIgnoreCase))
        {
            ApplyOnlineEffectMoveFromSelectionResult(result);
        }

        BattleActionType consumedActionType =
            string.Equals(
                result.resolvedEffectRef,
        ...
        ApplyBattleCountSnapshot(result);
        SetBattleBusy(false, "SelectEffectTargetResult applied");
        RefreshAllUI();

        SetSystemMessage(
            !string.IsNullOrWhiteSpace(result.message)
                ? result.message
                : "효과 대상 선택 결과가 적용되었습니다.");
    ...
    private void ApplyOnlineEffectMoveFromSelectionResult(BattleActionResult result)
    {
        ...
        targetSlot.SetCharacterBattleStats(
            ...
            result.characterCurrentTension);
        targetSlot.SetCharacterMovedThisTurn(result.characterMovedThisTurn);
        targetSlot.SetCharacterActiveUsedThisTurn(result.characterActiveUsedThisTurn);
        targetSlot.SetMovementLockedByBroadcastUntilTurn(movementLockedUntilTurn);
        targetSlot.SetCollabEffectsSilencedUntilTurn(collabEffectsSilencedUntilTurn);
    ...
    private void ApplyOnlineCardRevealDeltas(BattleActionResult result)
    {
        ...
        foreach (CardRevealDelta delta in result.cardRevealDeltas)
        {
            if (delta == null)
                continue;

            Debug.Log(
                $"[CardReveal][PublicSearch] actionSequence={result.actionSequence}, " +
                $"chainId={delta.chainId}, actor={delta.actor}, effectRef={delta.sourceEffectRef}, " +
                $"revealedCardId={delta.revealedCardId}, revealedCardInstanceId={delta.revealedCardInstanceId}, " +
                $"destination={delta.destinationZone}, visibility={delta.visibility}");

            bool isRevealSelected = string.Equals(
                delta.visibility,
                CardAcquisitionVisibility.RevealSelected.ToString(),
                StringComparison.OrdinalIgnoreCase);
            bool isToHand = string.Equals(delta.destinationZone, "Hand", StringComparison.OrdinalIgnoreCase);
            bool shouldShowToLocal = isRevealSelected &&
                isToHand &&
                delta.actor != BattleSlotOwner.My;

            if (!shouldShowToLocal)
                continue;

            ShowRevealedCardNotification(delta);
        }
    }

    private void ShowRevealedCardNotification(CardRevealDelta delta)
    {
        if (delta == null || cardQuestionPanel == null)
            return;

        BaseCardData sourceCard = FindCardById(delta.revealedCardId);
        BaseCardData displayCard = sourceCard != null
            ? CreateRuntimeCardInstance(sourceCard, delta.revealedCardOwner.ToString())
            : new BaseCardData
            {
                id = delta.revealedCardId,
                name = string.IsNullOrWhiteSpace(delta.displayName) ? "공개 카드" : delta.displayName,
                kind = "Unknown",
                cardInstanceId = delta.revealedCardInstanceId
            };

        if (!string.IsNullOrWhiteSpace(delta.revealedCardInstanceId))
            displayCard.cardInstanceId = delta.revealedCardInstanceId;

        string revealId = !string.IsNullOrWhiteSpace(delta.revealId)
            ? delta.revealId
            : $"{delta.actionSequence}_{delta.revealedCardInstanceId}";

        bool opened = cardQuestionPanel.TryShowRevealNotification(
            revealId,
            "상대가 다음의 카드를 패에 더했습니다.",
            new List<BaseCardData> { displayCard },
            () =>
            {
                Debug.Log($"[CardRevealUI][NoNetworkResponse] revealId={revealId}");
            });

        Debug.Log(
            $"[CardRevealUI][Show] cardId={delta.revealedCardId}, sourceActor={delta.actor}, " +
            $"blocking=False, opened={opened}");
    }

    public void ShowLocalSearchToHandRevealFromExternal(
        BattleSlotOwner actor,
        BaseCardData selectedCard,
        string sourceEffectRef)
    {
        ...
        string revealId = $"local_{turnCount}_{selectedCard.cardInstanceId}_reveal";
        Debug.Log(
            $"[CardAcquisition][Classify] effectRef={sourceEffectRef}, " +
            $"visibility={CardAcquisitionVisibility.RevealSelected}, sourceZone=Deck, destinationZone=Hand");
        Debug.Log(
            $"[CardReveal][PublicSearch] actionSequence=0, chainId=local, actor={actor}, " +
            $"effectRef={sourceEffectRef}, cardId={selectedCard.id}, " +
            $"cardInstanceId={selectedCard.cardInstanceId}, reason=SearchToHand");

        if (cardQuestionPanel == null)
            return;

        string message = actor == BattleSlotOwner.My
            ? "다음의 카드를 패에 더했습니다."
            : "상대가 다음의 카드를 패에 더했습니다.";

        cardQuestionPanel.TryShowRevealNotification(
            revealId,
            message,
            new List<BaseCardData> { selectedCard },
            () => Debug.Log($"[CardRevealUI][NoNetworkResponse] revealId={revealId}"));
    }
    ...
    private void ApplyOnlineCardDrawDeltas(BattleActionResult result)
    {
        ...
        foreach (CardDrawDelta delta in result.cardDrawDeltas)
        {
            ...
            if (!string.IsNullOrWhiteSpace(delta.cardInstanceId))
            {
                ...
                {
                    Debug.LogWarning(
                        $"[OnlinePrivatePayload] Received private card id for non-local owner. " +
                        $"owner={delta.owner}, actionSequence={result.actionSequence}, effectRef={result.resolvedEffectRef}");
                }
            }
            else
            {
                Debug.Log(
                    $"[OnlinePrivatePayload] Sanitized CardDrawDelta count-only. " +
                    $"owner={delta.owner}, actionSequence={result.actionSequence}, effectRef={result.resolvedEffectRef}, " +
                    "actualZoneMutation=False");
            }
    ...
    private void ApplyOnlineStatusDeltas(BattleActionResult result)
    {
        ...
        foreach (StatusDelta delta in result.statusDeltas)
        {
            if (delta == null)
                continue;
            ...
            if (string.Equals(delta.statusType, StatusTypeSilenceCharacterCollabThisTurn, StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    Debug.LogWarning(
                        $"[EffectDelta] Silence status target missing. slot={delta.targetSlotId}, card={delta.targetCardInstanceId}");
                    continue;
                }
            ...
            Debug.LogWarning(
                $"[EffectDelta] StatusDelta type is not implemented yet. type={delta.statusType}");
        }
    }
    ...
    private void ApplyOnlineActionStateDeltas(BattleActionResult result)
    {
        ...
        foreach (ActionStateDelta delta in result.actionStateDeltas)
        {
            ...
            if (string.Equals(delta.actionStateType, "CharacterActiveUsedThisTurn", StringComparison.OrdinalIgnoreCase))
            {
                BattleFieldSlot slot = FindSlotById(delta.slotId);
                if (slot != null && slot.HasCharacter)
                {
                    slot.SetCharacterActiveUsedThisTurn(delta.after);
                    Debug.Log(
                        $"[EffectDelta] CharacterActiveUsedThisTurn applied. owner={delta.owner}, " +
                        $"slot={delta.slotId}, before={delta.before}, after={delta.after}, turn={delta.turn}");
                }
                else
                {
                    Debug.LogWarning(
            ...
            Debug.LogWarning(
                $"[EffectDelta] ActionStateDelta type is not implemented yet. type={delta.actionStateType}");
        }
    }
    ...
    private void ApplyBattleCountSnapshot(BattleActionResult result)
    {
        ...
        Debug.Log(
            $"[OnlineZoneAuthority][ClientAfterApply] localActor={(localIsHost ? 1 : 2)}, " +
            $"actionSequence={result.actionSequence}, localActualHand={myPlayer?.hand?.Count ?? 0}, " +
            $"localHandIdsHash={HashCardInstanceIds(myPlayer?.hand)}, " +
            $"opponentPublicHandCount={expectedEnemyHand}, localDeckActual={myPlayer?.mainDeck?.Count ?? 0}, " +
            $"snapshotLocalHand={expectedMyHand}, snapshotOpponentHand={expectedEnemyHand}");

        int actualMyHand = myPlayer != null && myPlayer.hand != null ? myPlayer.hand.Count : 0;
        if (actualMyHand != expectedMyHand)
        {
            Debug.LogWarning(
                $"[OnlineZoneDesync] actor=My, actionSequence={result.actionSequence}, zone=Hand, " +
                $"actual={actualMyHand}, authoritativeCount={expectedMyHand}, " +
                "reason=CountMismatchAfterResult");
        }

        int actualMyDeck = myPlayer != null && myPlayer.mainDeck != null ? myPlayer.mainDeck.Count : 0;
        if (actualMyDeck != expectedMyDeck)
        {
            Debug.LogWarning(
                $"[OnlineZoneDesync] actor=My, actionSequence={result.actionSequence}, zone=MainDeck, " +
                $"actual={actualMyDeck}, authoritativeCount={expectedMyDeck}, " +
                "reason=CountMismatchAfterResult");
        }

        string duplicateHandId = FindDuplicateCardInstanceId(myPlayer?.hand);
        if (!string.IsNullOrWhiteSpace(duplicateHandId))
        {
            Debug.LogWarning(
                $"[OnlineZoneDesync] actor=My, actionSequence={result.actionSequence}, zone=Hand, " +
                $"actual={actualMyHand}, authoritativeCount={expectedMyHand}, " +
                $"reason=DuplicateInstanceId, card={duplicateHandId}");
        }
    }

    private string FindDuplicateCardInstanceId(List<BaseCardData> cards)
    {
        if (cards == null || cards.Count == 0)
            return "";
        ...
        foreach (BaseCardData card in cards)
        {
            string id = card != null ? card.cardInstanceId : "";
    ...
    private bool RequestSelectEffectTargetOnline(BattleAction action)
    {
        OnlineBattleSession session = OnlineBattleSession.Instance;
        if (session == null)
            session = FindAnyObjectByType<OnlineBattleSession>();
        ...
        ClearPendingFieldSlotSelection(false);
        activeOnlineSelectionRequest = null;
        SetBattleBusy(true, "OnlineSelectionResponsePending");
        RefreshTurnEndButtonState();
        SetSystemMessage("Host의 효과 대상 판정을 기다리고 있습니다.");
        Debug.Log(
            $"[OnlineSelection] SelectEffectTarget sent. requestId={action.selectionRequestId}, " +
            $"effectRef={action.effectRef}, targetSlot={action.targetSlotId}");
        return true;
    }
    ...
    {
        return CanUseCharacterActiveForOwner(slot, BattleSlotOwner.My, out failReason);
    }

    private bool CanUseCharacterActiveForOwner(
        BattleFieldSlot slot,
        BattleSlotOwner owner,
        out string failReason)
    {
        ...
        {
            failReason = owner == BattleSlotOwner.My
                ? "상대 캐릭터의 효과는 발동할 수 없습니다."
                : "해당 플레이어가 조종하는 캐릭터의 효과만 발동할 수 있습니다.";
            return false;
        }
        ...
        else
        {
            ...
            if (currentActionSide != expectedSide)
            {
                failReason = "현재 행동권 플레이어가 아닙니다.";
                return false;
            }

            if (IsBattleBusy())
            {
                failReason = battleBusyReason;
                return false;
            }
        }
    ...
    public bool RequestUseCharacterActiveActionFromExternal(BattleFieldSlot slot)
    {
        ...
        if (BattleStartSettings.IsOnlineBattle)
            return RequestUseCharacterActiveOnline(action);
    ...
    private bool RequestUseCharacterActiveOnline(BattleAction action)
    {
        if (onlineUseCharacterActiveRequestPending)
        {
            SetSystemMessage("Host의 캐릭터 액티브 판정을 기다리고 있습니다.");
            return false;
        }
        ...
        if (string.IsNullOrWhiteSpace(json) || !session.SendBattleActionRequest(json))
        {
            SetSystemMessage("캐릭터 액티브 요청 전송에 실패했습니다.");
            return false;
        }

        onlineUseCharacterActiveRequestPending = true;
        RefreshTurnEndButtonState();
        SetSystemMessage("Host의 캐릭터 액티브 판정을 기다리고 있습니다.");
        Debug.Log(
            $"[OnlineCharacterActive] UseCharacterActiveAction sent. actor={action.actor}, " +
            $"card={action.cardInstanceId}, sourceSlot={action.sourceSlotId}, effectRef={action.effectRef}");
        return true;
    }
    ...
    public bool CanUseIdolActiveFromExternal(BattleSlotOwner owner, out string failReason)
    {
        return CanUseIdolActive(owner, out failReason);
    }
    ...
    public bool RequestUseIdolActiveActionFromExternal(BattleSlotOwner owner)
    {
        ...
        if (BattleStartSettings.IsOnlineBattle)
            return RequestUseIdolActiveOnline(action);
    ...
    private bool RequestUseIdolActiveOnline(BattleAction action)
    {
        if (onlineUseIdolActiveRequestPending)
        {
            SetSystemMessage("Host의 아이돌 액티브 판정을 기다리고 있습니다.");
            return false;
        }
        ...
        if (string.IsNullOrWhiteSpace(json) || !session.SendBattleActionRequest(json))
        {
            SetSystemMessage("아이돌 액티브 요청 전송에 실패했습니다.");
            return false;
        }

        onlineUseIdolActiveRequestPending = true;
        RefreshTurnEndButtonState();
        SetSystemMessage("Host의 아이돌 액티브 판정을 기다리고 있습니다.");
        Debug.Log(
            $"[OnlineIdolActive] UseIdolActiveAction sent. actor={action.actor}, " +
            $"card={action.cardInstanceId}, effectRef={action.effectRef}");
        return true;
    }
        ...
        string cardInstanceId)
    {
        ...
        if (deckIndex < 0)
        {
            if (ContainsCardInstance(player.hand, cardInstanceId))
            {
                Debug.Log(
                    $"[OnlineBattle] Draw card already in hand; skip duplicate apply. " +
                    $"owner={owner}, cardInstanceId={cardInstanceId}");
                return true;
            }
    ...
    private void RefreshTurnEndButtonState()
    {
        ...
        if (turnEndButton != null)
        {
            turnEndButton.interactable =
                ...
                !onlineUseCharacterActiveRequestPending &&
                currentPhase == BattlePhase.MainGame &&
                currentActionSide == BattlePlayerSide.My &&
